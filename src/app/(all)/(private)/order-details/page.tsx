...
import { fetchOrderById } from "@/rtk/slices/orders/ordersSlice";
import {
  MapPin,
  Package,
  User,
...
export default function OrderDetailsPage() {
  ...
  const { selectedOrder, selectedLoading } = useAppSelector((state) => state.order);
  ...
  return (
    <div className="container py-8 mt-5 mb-20">
      {/* Order Header */}
      <div className="mb-8">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.product.brand?.name && (
                      <p className="text-xs text-gray-400 mt-1 truncate">{item.product.brand.name}</p>
                    )}
          ...
          <div className="text-center">
            {order.status === 1 ? (
              <button
                ...
                disabled={cancelling}
                className="m-auto rounded-full flex w-1/2 items-center justify-center gap-1.5 px-3 py-2 text-sm font-medium text-rose-700 bg-rose-50 border border-rose-200 hover:bg-rose-100 hover:text-rose-800 hover:border-rose-300 transition-all disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-rose-500/50"
              >
                {cancelling ? (
                  <>
