...
export default function OrderDetailsPage() {
    ...
    const itemSeller = order.orderProducts.find(
        (o) => String(o.product?.seller?.id) === user?.id
    );
    ...
    return (
        <div className="mx-4 md:mx-6 lg:mx-8  py-8 mt-5 mb-20">
            ...
            <div className="mb-8">
                ...
                {/* Action + Status */}
                <div className="flex items-center gap-3">
                    {/* Only show button if seller matches */}
                    {itemSeller && itemSeller.status != 2 && (
                        <button
                            onClick={() => dispatch(markOrderProductReady({ orderProductId: itemSeller.id }))}
                            disabled={productLoading[itemSeller.id]}
                            className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium border text-white bg-indigo-600 hover:bg-indigo-700 transition-colors ${productLoading[itemSeller.id] ? "opacity-50 cursor-not-allowed" : ""
                                }`}
                        >
                    ...
                    {
                        itemSeller?.status && (
                            <StatusBadge status={itemSeller.status} />
                        )
                    }
                </div>
                ...
                <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                        ...
                        <div className="divide-y divide-gray-100">
                            {order.orderProducts
                                // .filter((o) => o.product?.seller?.id === Number(user?.id))
                                .map((item) => (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex justify-between items-center w-full">
                                                ...
                                                <h4
                                                    ...
                                                    title={item.product.name} // shows full name on hover
                                                >
                                                    {item.product.name}
                                                </h4>
                                                ...
                                                <div className="flex items-center gap-2 sm:gap-3">
                                                    {/* Owner Badge */}
                                                    {String(item.product.seller?.id) === user?.id && (
